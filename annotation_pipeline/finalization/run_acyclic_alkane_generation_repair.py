...
SYSTEM_PROMPT = r"""
You are an extremely experienced **EI (70 eV) mass spectrometry interpretation expert**, specializing in the **fragmentation decision logic of acyclic_alkane compounds under EI conditions**.
...
────────────────────────────────
IV. Overall Spectral Observation and Anomalous-Peak Determination
────────────────────────────────
...
────────────────────────────────
V. Weak Peak → Strong Peak Prohibition Rule
────────────────────────────────
...
────────────────────────────────
VI. Fragmentation Mechanism Types
────────────────────────────────
...
────────────────────────────────
VIII. Hard Format for Precursor
────────────────────────────────
A) If origin_type="structure-driven"
- The precursor must begin with "smiles_fragment: "
- What follows must be a real substring of the input SMILES
...
【Precursor output format for structure-driven cases】
...
Where:
- Both <best_fragment> and <alt\#> must be findable in source_smiles as substrings
...
- best_fragment must satisfy:
  1) The fragment must be a substring of source_smiles
  2) The carbon number of the fragment must be chemically compatible with the carbon number of product_ion;
     for acyclic_acyclic_alkane fragments: preferentially select a fragment whose carbon number equals the ion carbon number;
...
────────────────────────────────
X. Minimum Format Requirement for product_ion
────────────────────────────────
...
────────────────────────────────
XII. Structural Upgrade Determination for “Anomalously High Even m/z Peaks” in acyclic_alkane
────────────────────────────────
Applicable objects: mainly acyclic_alkane / branched acyclic_alkane compounds (only C/H; no heteroatom-dominated fragmentation)

【Background knowledge constraint】
- In acyclic_alkane EI, strong peaks usually fall in the “odd m/z” alkyl cation series (C_nH_{2n+1}+).
...
────────────────────────────────
XIII. Parallel Terminal-State Output for “Same-Mass Fragments with Multiple Mechanisms”
────────────────────────────────
...
────────────────────────────────
XIV. Mandatory non-empty repair mode
────────────────────────────────
...
CURRENT_RESULT_FILE = "../data/acyclic_alkane_outputs/openai_gpt41_test_results.json"
OUTPUT_FILE = "./acyclic_alkane_outputs/openai_gpt41_test_results_repaired.json"
FAILURE_LOG_FILE = "./acyclic_alkane_outputs/openai_gpt41_test_results_repair_log.json"
...
def call_nuwa_with_retry(
    ...
    token_attempts = token_attempts[: max(1, int(API_RETRIES))]
...
def main() -> None:
    ...
    old_path = os.path.abspath(CURRENT_RESULT_FILE)
    ...
    source_datas = load_json(INPUT_FILE)
    ...
    old_results = load_json(CURRENT_RESULT_FILE)
    if not isinstance(old_results, list):
        raise ValueError("--current_result_file must contain a JSON list.")

    if os.path.exists(OUTPUT_FILE):
        working_results = load_json(OUTPUT_FILE)
        if not isinstance(working_results, list):
            raise ValueError("Existing --output_file must contain a JSON list.")
        print(f"[RESUME] Loaded merged output: {OUTPUT_FILE}")
    else:
        working_results = copy.deepcopy(old_results)
        save_json_atomic(working_results, OUTPUT_FILE)
        print(f"[INIT] Copied old result to new merged output: {OUTPUT_FILE}")

    failure_log_path = FAILURE_LOG_FILE or default_failure_log(OUTPUT_FILE)
    failure_state = load_failure_state(failure_log_path)
    failure_state["current_result_file"] = CURRENT_RESULT_FILE
    failure_state["output_file"] = OUTPUT_FILE
    failure_state["base_url"] = BASE_URL
    failure_state["model"] = MODEL
    ...
    if START:
        repair_targets = repair_targets[START :]
    if LIMIT is not None:
        repair_targets = repair_targets[: LIMIT]

    print("=" * 92)
    print(f"Nuwa base URL:                  {BASE_URL}")
    print(f"Model:                          {MODEL}")
    print(f"Old result (read-only):         {CURRENT_RESULT_FILE}")
    print(f"New merged output:              {OUTPUT_FILE}")
    ...
    for order, (result_pos, target_key, target_mz, _) in enumerate(repair_targets, start=1):
        ...
        for semantic_attempt in range(1, REPAIR_ATTEMPTS + 1):
            ...
            print(
                f"[Semantic repair {semantic_attempt}/{REPAIR_ATTEMPTS}] "
        ...
        print(
            ...
            f"fallback={used_fallback}, output={OUTPUT_FILE}"
        )
    ...
    print(f"Old result remains unchanged:   {CURRENT_RESULT_FILE}")
    print(f"Merged repaired output:         {OUTPUT_FILE}")
